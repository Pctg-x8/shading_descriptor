...
use {GenSource, TyDeformerIntermediate, Lambda};
...
{
    pub name: GenSource<'s, 't>, pub ty: TyDeformerIntermediate<'s, 't>, pub expressed: Lambda<'s, 't>
}
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DataConstructorScope<'s: 't, 't>
{
    pub name: GenSource<'s, 't>, pub ty_expressed: TyDeformerIntermediate<'s, 't>,
    pub ctors: Vec<DataConstructor<'s, 't>>
}
...
pub trait SymbolScope<'s: 't, 't>
{
    ...
    fn register_value<N: Into<Cow<'t, str>>>(&mut self, name: N, span: GenSource<'s, 't>) -> Result<(), &SymbolKind<'s, 't>>
    {
    ...
    fn register_type<N: Into<Cow<'t, str>>>(&mut self, name: N, span: GenSource<'s, 't>) -> Result<(), &SymbolKind<'s, 't>>
    {
        if self.symbol_map().contains_key(&name) { Err(self.symbol_map().get(&name).unwrap()) }
        else { self.symbol_map_mut().insert(name.into(), SymbolKind::Type(span)); Ok(()) }
    }
    fn register_data<N: Into<Cow<'t, str>>>(&mut self, name: N, scope: DataConstructorScope<'s, 't>) -> Result<(), &SymbolKind<'s, 't>>
    {
        if self.symbol_map().contains_key(&name) { Err(self.symbol_map().get(&name).unwrap()) }
...
pub struct SymbolDuplicateError<'s: 't, 't> { pub org_pos: &'t Location, pub pos: &'t Location, pub name: &'s str, pub org_kind: &'static str }
...
impl<'s: 't, 't> SymbolCollector<'s, 't> for ShaderStageDefinition<'s>
{
    type Map = ShaderStageSymbols<'s, 't>;
    fn collect_symbols(&'t self) -> Result<Self::Map, Vec<String>>
    {
        ...
        {
            if let Err(e) = symbols.register_value(name, GenSource::Sliced(name))
            {
                errors.place_back() <- SymbolDuplicateError { pos: location, name, org_kind: e.kind_text(), org_pos: e.position() };
            }
        }
